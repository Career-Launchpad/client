...
const getAvgWageForMajor = (offers, students, major) => {
  ...
  const getNumOffersForMajor = (offers, students, major) => {
    let n_offers = 0;
    let major_students = students.edges.filter(student => student.major === major).map(student => {return student.id});
  
    for(let i in offers.edges) {
      if (major_students.includes(offers.edges[i].student_id)) { n_offers += 1; }
    }
...
export const OffersPerMajorBarGraph = ( majors, students, offers) => {
    ...
    majors.forEach((major, i) => {
      numStudents.push(students.edges.filter(student => student.major === major).length);
